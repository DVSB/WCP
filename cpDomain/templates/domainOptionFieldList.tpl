	...
	{if $option.isOptionGroup|isset}
		<div class="{if $divClass}{$divClass} {/if}formGroup" id="{$option.optionName}Div">
			<div class="formGroupLabel">
				<label>{lang}cp.domain.option.{$option.optionName}{/lang}</label>
			</div>
			<div class="formGroupField">
				<fieldset>
					<legend>{lang}cp.domain.option.{$option.optionName}{/lang}</legend>
	...
	{else}
		<div class="{if $divClass}{$divClass} {/if}formElement" id="{$option.optionName}Div">
				...
				{if $error}
					<p class="innerError">
						{if $error == 'empty'}
							{lang}wcf.global.error.empty{/lang}
						{else}	
							{lang}cp.domain.option.error.{$error}{/lang}
						{/if}
					</p>
				{/if}
			{/capture}
			...
			{if $option.beforeLabel}
				<div class="formField">
					<label for="{$option.optionName}">{@$option.html} {lang}cp.domain.option.{$option.optionName}{/lang}</label>
					{@$innerError}
				</div>
			{else}
				<div class="formFieldLabel">
					<label for="{$option.optionName}">{lang}cp.domain.option.{$option.optionName}{/lang}</label>
				</div>
				<div class="formField">
					{@$option.html}
					{@$innerError}			
